...
public class EmpleadoDAO {
	...
	public void modificarEmpleado(Empleado empleado, String nombreAnterior, int dniCuitAnterior) throws ConexionException, AccesoException{
		...
		String SQL = ("UPDATE clientes SET nombre =('" + empleado.getNombre() +"'), password=('" + empleado.getPassword() +"'), nroLu = ('" + empleado.getNroLU() +"'), rolOriginal = ('" + empleado.getRolOriginal() +"'), rolTemporal = ('" + empleado.getRolTemporal() +"') WHERE nombreUsr = ('" + empleado.getNomUsr() +"');");
		try{
			stmt.execute(SQL);
		} catch (SQLException e1) {
			System.out.println(e1.getMessage());
